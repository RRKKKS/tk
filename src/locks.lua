...
data = {
{
{text = '- نعم ', data =msg.sender_id.user_id..'/Bina'},{text = '- لا ', data =msg.sender_id.user_id..'/ZLLKA'}, 
},
}
}
return send(msg.chat_id,msg.id,'*\n● - هل انت متأكد بطردك من الجروب - *',"md",false, false, false, false, reply_markup)
end
if text and text:match("(.*)(مين ضافني)(.*)") then
local StatusMember = bot.getChatMember(msg_chat_id,msg.sender_id.user_id).status.luatele
if (StatusMember == "chatMemberStatusCreator") then
return send(msg_chat_id,msg_id,"● انت منشئ الجروب","md",true) 
end
local Added_Me = Redis:get(Fast.."Who:Added:Me"..msg.sender_id.user_id..':'..msg.sender_id.user_id)
if Added_Me then 
ban = bot.getUser(Added_Me)
...
data = {
{
{text = 'مغادره الجروب ⎋', data =msg.sender_id.user_id..'/Bina'},
},
}
}
return send(msg.chat_id,msg.id,'*\nانت دخلت عن طريق الرابط ⎋*',"md",false, false, false, false, reply_markup)
end
end
